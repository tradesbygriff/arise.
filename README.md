...
5. If you change the service worker's pre-cached shell behavior or want to force removal of an older ARISE cache immediately, increment `CACHE_NAME` in `service-worker.js` (for example, from `arise-pwa-v2` to `arise-pwa-v3`). Obsolete `arise-pwa-*` caches are deleted during activation.
